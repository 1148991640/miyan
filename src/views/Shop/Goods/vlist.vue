  ...
  <div class="table-bg">
    <el-table :data="goodslist">
      ...
      <el-table-column label="规格名" align="center">
        <template slot-scope="scope" v-for="(item,index) in goodslist">
          <el-tag v-if="item.id==scope.row.goodsid" :key="index" type="success">{{item.goodsname}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="规格值" align="center">
        <template slot-scope="scope">
          <el-tag v-for="(item,index) in scope.row.goodsattr" :key="index" type="success">{{item}}</el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="price" label="价格" align="center"></el-table-column>
      <el-table-column lable="是否热卖">
        <template slot-scope="scope">
          <el-switch v-model="scope.row.ishot" :active-value="1" :inactive-value="2"></el-switch>
        </template>
      </el-table-column>
      <el-table-column lable="是否新品">
        <template slot-scope="scope">
    ...
    <el-pagination background @size-change="set_size" @current-change="set_page" :current-page="page" :page-sizes="[1, 2, 3, 4]" :page-size="size" layout="total, sizes, prev, pager, next, jumper" :total="total">
    </el-pagination>
  </div>
</template>
<script>
